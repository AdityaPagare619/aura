...
impl ProactiveEngine {
    ...
    /// Regenerate initiative budget based on elapsed seconds.
    pub fn regenerate_initiative(&mut self, elapsed_secs: f32) {
        if elapsed_secs <= 0.0 {
            return;
        }
        let gain = elapsed_secs * INITIATIVE_REGEN_RATE;
        self.initiative_budget = (self.initiative_budget + gain).min(MAX_INITIATIVE_BUDGET);
        debug!(
            budget = self.initiative_budget,
            gain, "initiative regenerated"
        );
    }
    ...
    pub fn tick(
        ...
        self.regenerate_initiative(elapsed_secs);
...
mod tests {
    ...
    fn test_regenerate_initiative() {
        ...
        e.regenerate_initiative(100.0); // 100s * 0.001 = 0.1
    ...
    fn test_regenerate_capped_at_max() {
        ...
        e.regenerate_initiative(1000.0); // would add 1.0
        assert!(
    ...
    fn test_tick_returns_ok_empty() {
        let mut e = ProactiveEngine::new();
        let result = e.tick(1_000, PowerTier::P2Normal, ContextMode::Default, true);
        assert!(result.is_ok());
    ...
    fn test_tick_sleeping_suppresses_suggestions() {
        let mut e = ProactiveEngine::new();
        let actions = e
            .tick(1_000, PowerTier::P0Always, ContextMode::Sleeping, true)
            .expect("tick ok");
