...
pub async fn run(mut state: DaemonState) {
    ...
    // Clone TX halves BEFORE split for channels that need internal producers.
    ...
    tracing::info!(
        interval_secs = CRON_TICK_INTERVAL_SECS,
        "cron scheduler spawned"
    );

    // Spawn the heartbeat loop — fires health events with real device metrics.
    let _health_handle = tokio::spawn(crate::health::monitor::run_heartbeat_loop(
        now_ms(),
        0,
        health_tx,
        state.cancel_flag.clone(),
    ));
    tracing::info!("heartbeat loop spawned");
                                ...
                                match boundary_result {
                                    ...
                                    BoundaryGateResult::Allow => {
                                            ...
                                            check_task_policy_gate(subs, &desc)
                                        {
                                            ...
                                            let (outcome, _session) = subs
                                                .react_engine
                                                .execute_task(desc.clone(), 7, None)
                                                .await;

                                            let (result_kind, duration_ms, confidence) =
                                                match &outcome {
                                                    react::TaskOutcome::Success {
                                                        total_ms,
                                                        final_confidence,
